...
async function setStorageData(d){
  return new Promise(async function(resolve,reject){
    storageData = d;
    return resolve(storageData);
  })
}
async function getStorageData(d){
  return new Promise(async function(resolve,reject){
    return resolve(storageData);
  })
}
...
async function sysStartUp(){
  return new Promise(async function(resolve,reject){
    ...
    if(checkFile){
      await setStorageData(await getTheData(working_file))
    }else{
      ...
      await setStorageData(await createDummyData());
    }
  });
}
//
